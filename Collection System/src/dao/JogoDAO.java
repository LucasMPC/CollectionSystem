...
public class JogoDAO {
    ...
    // 6. Método auxiliar para descobrir o nome da coleção a partir do ID do jogo
    ...
    // 7. PESQUISAR: Busca jogos por nome dentro de uma coleção específica
    public java.util.List<Jogo> pesquisarPorNome(int idColecao, String termoPesquisa) {
        java.util.List<Jogo> lista = new java.util.ArrayList<>();
        
        // O "LIKE" permite buscar pedaços do texto
        String sql = "SELECT j.*, d.nome AS nome_dev " +
                     "FROM jogo j " +
                     "LEFT JOIN desenvolvedora d ON j.id_desenvolvedora = d.id " +
                     "WHERE j.id_colecao = ? AND j.nome LIKE ?";
        
        try (Connection conn = Conexao.getConexao();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            
            ps.setInt(1, idColecao);
            // Adiciona as porcentagens (%) para buscar em qualquer parte do nome
            ps.setString(2, "%" + termoPesquisa + "%");
            
            java.sql.ResultSet rs = ps.executeQuery();
            
            while (rs.next()) {
                // Reaproveitando a lógica de criar objeto Jogo
                int idJogo = rs.getInt("id");
                String nome = rs.getString("nome");
                String data = rs.getString("data_lancamento");
                String desc = rs.getString("descricao");
                ...
                String nomeDev = rs.getString("nome_dev");
                if (nomeDev == null) nomeDev = "N/A";
                
                Desenvolvedora dev = new Desenvolvedora(rs.getInt("id_desenvolvedora"), nomeDev);
